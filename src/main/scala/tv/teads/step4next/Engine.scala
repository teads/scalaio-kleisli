...
object Engine {
  ...
  object SyncRule {
    ...
    implicit val monoidK: MonoidK[SyncRule] = new MonoidK[SyncRule] {
      override def empty[A]: SyncRule[A] = {
        SyncRule(t => Right(t))
      }

      override def combineK[A](x: SyncRule[A], y: SyncRule[A]): SyncRule[A] = {
        SyncRule(t => x(t).flatMap(value => y(value)))
      }
    }
